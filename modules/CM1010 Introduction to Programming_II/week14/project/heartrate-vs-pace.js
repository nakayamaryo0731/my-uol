...
// Heart Rate vs Pace: Scatter plot with statistical analysis showing the
// relationship between heart rate and running pace. Calculates Pearson's
// correlation coefficient and draws a least-squares linear regression line.
// Uses composition with ScatterPlot for point rendering and Tooltip for
// hover interaction.
function HeartRateVsPace() {
  ...
  // Single-pass algorithm: O(n) time complexity where n = number of data points.
  ...
  this.draw = function() {
    ...
    // Map distance to point size: longer runs get larger bubbles
    var distances = this.processedData.distances;
    var minDist = min(distances);
    var maxDist = max(distances);
    var sizes = [];
    for (var i = 0; i < distances.length; i++) {
      sizes.push(map(distances[i], minDist, maxDist, 4, 28));
    }
