...
// Pace Progress: Displays running pace over time with individual data points
// and a 10-run trailing moving average trend line. Filters runs by minimum
// distance (3 km) and valid pace range (4–10 min/km) to ensure data quality.
function PaceProgress() {
  ...
  this.processData = function() {
    ...
    for (var i = 0; i < this.data.getRowCount(); i++) {
      ...
      if (activityType === 'Run') {
        ...
        if (distance >= 3) {
          var paceMinutes = paceToMinutes(paceStr);
  ...
  // O(n * w) time complexity where n = data length, w = window size.
  ...
  // Draw colour-coded background bands for pace zones
  this.drawPaceZones = function() {
    noStroke();
    var plotLeft = this.layout.leftMargin;
    var plotWidth = this.layout.rightMargin - this.layout.leftMargin;

    var zones = [
      { min: this.minPace, max: 6, col: [144, 238, 144, 30], label: 'Fast' },
      { min: 6, max: 7, col: [255, 236, 153, 30], label: 'Moderate' },
      { min: 7, max: this.maxPace, col: [255, 180, 150, 30], label: 'Easy' }
    ];

    for (var i = 0; i < zones.length; i++) {
      var zone = zones[i];
      if (zone.min >= this.maxPace || zone.max <= this.minPace) continue;

      var clampedMin = Math.max(zone.min, this.minPace);
      var clampedMax = Math.min(zone.max, this.maxPace);
      var yTop = this.mapPaceToHeight(clampedMin);
      var yBottom = this.mapPaceToHeight(clampedMax);

      fill(zone.col[0], zone.col[1], zone.col[2], zone.col[3]);
      rect(plotLeft, yTop, plotWidth, yBottom - yTop);

      fill(100, 80);
      textSize(9);
      textAlign('right', 'center');
      text(zone.label, this.layout.rightMargin - 5, (yTop + yBottom) / 2);
    }
  };
