/**
 * CM1005 - Game Project 6 (Complete Skateboard Game)
 * 
 * 主な仕様：
...
function draw()
{
	...
	// ===== ワールド座標系での描画 =====
	// push/translate/popを使用してカメラスクロールを実装
	push();
...
/**
 * ゲーム状態テキスト表示
 * Game OverまたはLevel Completeのテキストを表示する
 */
...
function keyControl(event) {
	...
	// ゲーム終了時はスペースキーでリスタートのみ許可
	if((gameOver || levelComplete) && key === ' ' && isKeyPressed) {
		// ゲーム完全リスタート
		game_score = 0;
		lives = 3;
		gameOver = false;
